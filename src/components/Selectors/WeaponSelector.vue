<template>
  <div>
    ...
    <p v-for="(weapon, idx) in addedWeapons" :key="idx">
      Name: {{ weapon.name }} Mass: {{ weapon.mass }} Repair Cost:
      {{ weapon.repairCost }} Energy Cost: {{ weapon.energyCost }} Range:
      {{ weapon.range }} Damage: {{ weapon.damage }} Ammunition Type:
      {{ weapon.ammunitionType }} Munition Velocity:
      {{ weapon.munitionVelocity }}
  ...
  methods: {
    ...
    addWeapon() {
      this.$set(
        this.addedWeapons,
        this.weaponIndex,
        this.listedWeapons[this.selectedWeapon]
      );
      this.weaponIndex++;
      this.$store.commit(
        "setShipWeapon",
